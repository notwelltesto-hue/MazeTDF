...
const url = (location.hostname === '') ? 'ws://localhost:3000' : `ws://${location.hostname}:3000`;
...
// input
window.addEventListener('keydown',(e)=>{ if(e.key==='w') inputs.up=true; if(e.key==='s') inputs.down=true; if(e.key==='a') inputs.left=true; if(e.key==='d') inputs.right=true; });
...
function sendInputs(){
  inputs.seq++;
  if(ws && ws.readyState===WebSocket.OPEN){
    ws.send(JSON.stringify({type:'input', seq: inputs.seq, up:inputs.up, down:inputs.down, left:inputs.left, right:inputs.right, rot:inputs.rot, shooting:inputs.shooting, shootingSeq: inputs.shootingSeq}));
  }
}

// simple camera: follow my player
...
function draw(){
  ...
  state.resources.forEach(r=>{
    ...
    ctx.beginPath();
    ctx.ellipse(s.x, s.y, radius, radius, 0, 0, Math.PI*2);
    ctx.fill();
  });
